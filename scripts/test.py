...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        default=['pa-mpjpe', 'mpjpe', 'pve', 'pa-pve'],  #'miou' pmiou
...
def main():
    ...
    if args.checkpoint:
        ...
        model.init_weights()
    use_miou = 'miou' in args.metrics
    use_pmiou = 'pmiou' in args.metrics
    if not distributed:
        if args.device == 'cpu':
            model = model.cpu()
        else:
        ...
        model.module.vis = args.vis
        model.module.miou = use_miou
        model.module.pmiou = use_pmiou
