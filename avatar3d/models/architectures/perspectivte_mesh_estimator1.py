...
from avatar3d.transforms.transform3d import rotmat_to_rot6d, ee_to_rotmat
...
class PersepectiveMeshEstimator1(BaseArchitecture):
    ...
    def __init__(
        ...
        super(PersepectiveMeshEstimator1, self).__init__(init_cfg)
    ...
    def compute_losses(self, predictions: dict, targets: dict):
        ...
        if self.loss_keypoints2d_spec is not None:
            ...
            # pred_focal_length[no_transl_ids] = 1000 / 112

            # from avatar3d.utils.torch_utils import image_tensor2numpy
            # import cv2
            # index=0

            # im = image_tensor2numpy(targets['full_img'].permute(0, 2, 3, 1))
            # cv2.imwrite(f'{index}_full.png', im[index])
            ...
            orig_focal_length = pred_focal_length * scale / 2
            ...
            # from avatar3d.cameras.utils import estimate_cam_weakperspective_batch
            ...
            #     joints2d_conf, 224)
            # gt_transl = pred_cam_to_full_transl(
            #     gt_cam, center, scale, targets['ori_shape'],
            #     orig_focal_length).float()
        ...
        if self.loss_keypoints2d_prompt is not None:
            ...
            target_focal_length_ndc = targets['orig_focal_length'].view(
                -1) * 2 / scale
            target_focal_length_ndc[no_transl_ids] = 1000 / 112

            orig_focal_length = target_focal_length_ndc.view(-1) * scale / 2

            if self.use_pred_transl:
                pred_transl = pred_cam_to_full_transl(
                    pred_cam, center, scale, targets['ori_shape'],
                    orig_focal_length).float()

            else:
                target_transl = targets['smpl_transl']
                ...
                computed_transl = pred_cam_to_full_transl(
                    gt_cam, center, scale, targets['ori_shape'],
                    orig_focal_length)
                target_transl[no_transl_ids, :] = computed_transl[
                    no_transl_ids, :].float()
                pred_transl = target_transl
            ...
            # # K[:, 0, 0] = targets['sm']
            # # K[:, 1, 1] = target_transl[index, 2:3] * pred_cam[index,  0:1]
            ...
            # rgb = image_tensor2numpy(targets['img'][index].permute(1, 2, 0))
            # cv2.imwrite(f'{index}_rgb.png', rgb)
            # vis_smpl(verts=pred_vertices[index] +
            #          targets['smpl_transl'][index].view(1, 1, 3),
            #          cameras=cameras,
            #          device=device,
            ...
            #          overwrite=True,
            #          no_grad=True)
            # vis_smpl(verts=gt_vertices[index] +
            #          targets['smpl_transl'][index].view(1, 1, 3),
            #          cameras=cameras,
            #          device=device,
            #          body_model=self.body_model_train,
            #          output_path=f'{index}_gt.png',
            #          return_tensor=False,
            #          overwrite=True,
            #          no_grad=True)
            # gt = cv2.imread(f'{index}_gt.png')
            # im = draw_skeletons_image(
            #     gt_keypoints2d[index].detach().cpu().numpy(),
            #     gt,
            #     convention='smpl_54')
            # cv2.imwrite(f'{index}_kp2d.png', im)

            # im=cv2.imread(targets['img_metas'][index]['image_path'])
            # cv2.imwrite(f'{index}_orig.png',im)
            # kp2d = targets['origin_keypoints2d'][index].detach().cpu().numpy()
            # if targets['is_flipped'][index]:
            #     im = draw_skeletons_image(
            #         kp2d,
            #         np.flip(im, axis=1),
            #         convention='smpl_54')
            # cv2.imwrite(f'{index}_orig_kp2d.png', im)
            ...
            if self.use_d_weight:
                ...
                for idx in has_uvd_ids:
                    for kp_idx in range(gt_keypoints2d.shape[1]):
                        x = int(gt_keypoints2d[idx, kp_idx, 0])
                        y = int(gt_keypoints2d[idx, kp_idx, 1])
                        if 0 < x < 224 and 0 < y < 224:
                            kp2d_weight[idx, kp_idx] = targets['d_img'][idx,
                                                                        0][y,
                                                                           x]
            #     kp2d_weight = torch.clip(kp2d_weight, 0.5, 5)
            # from avatar3d.utils.demo_utils import draw_skeletons_image
        ...
        if self.loss_camera is not None:
            ...
            gt_cam = estimate_cam_weakperspective_batch(
                gt_model_joints, targets['keypoints2d'], joints2d_conf,
                joints2d_conf, 224)
            losses['camera_loss'] = self.compute_camera_loss(pred_cam, gt_cam)
        ...
        if self.loss_transl_z is not None:
            ...
            has_transl = targets['has_transl']
            transl_weight = has_transl
            gt_transl = targets['smpl_transl']
            transl_weight = 1. / gt_transl[:, 2]
            pred_z = predictions['pred_z']
            # transl_weight = transl_weight.view(-1, 1) * has_transl.view(-1, 1)
    ...
    def prepare_targets(self, data_batch: dict):
        if self.iuvd_head is not None:
            ...
            if not has_focal_length.all():
                ...
                gt_focal_length_ndc = data_batch['orig_focal_length'].view(
                    -1) * 2 / scale
                gt_focal_length_ndc[no_f_ids] = 1000 / 112
            ...
            has_uvd_ids = torch.where(has_uvd > 0)[0]

            if len(has_uvd_ids):
                uv_res = int(gt_iuv_img.shape[2])
                ori_shape = data_batch['ori_shape']
                orig_focal_length = data_batch['orig_focal_length'].view(-1)
                gt_iuv_img_distored, gt_d_img_distored = self.render_gt_iuvd_distorted(
                    gt_vertices[has_uvd_ids], gt_transl[has_uvd_ids],
                    center[has_uvd_ids], ori_shape[has_uvd_ids],
                    scale[has_uvd_ids], orig_focal_length[has_uvd_ids], uv_res)
                gt_iuv_img[has_uvd_ids] = gt_iuv_img_distored
                gt_d_img[has_uvd_ids] = gt_d_img_distored
            ...
            if not has_uvd.all():
                ...
                if len(render_ids):
                    ...
                    gt_iuv_img_online, gt_d_img_online = self.render_gt_iuvd(
                        ...
                        img_res=uv_res,
                        focal_length=gt_focal_length_ndc[render_ids] * uv_res /
                        2)
    ...
    def forward_test(self, **data_batch):
        ...
        if self.hmr_head is not None:
            ...
            if 'pred_z' in self.head_keys:
                data_batch['pred_z'] = predictions['pred_z']
            elif 'gt_z' in self.head_keys:
                if 'pred_z' in predictions:
                    self.head_keys = list(self.head_keys)
                    self.head_keys[self.head_keys.index('gt_z')] = 'pred_z'
                    data_batch['pred_z'] = predictions['pred_z']
                else:
                    self.head_keys.remove('gt_z')
            if 'warped_d_img' in self.head_keys:
                pred_d_img = predictions['pred_d_img']
                pred_iuv_img = predictions['pred_iuv_img']
                uv_renderer = self.uv_renderer.to(pred_d_img.device)
            ...
            # if 'polar_orient' in self.head_keys:
            #     transl = data_batch['pred_transl']
            #     polar_rotmat = ee_to_rotmat(
            #         torch.cat([
            #             torch.arctan(transl[:, 1:2] / transl[:, 2:3]),
            #             torch.arctan(transl[:, 0:1] / transl[:, 2:3]),
            #             torch.zeros_like(transl[:, 2:3])
            #         ], -1).view(-1, 3))
            #     polar_orient = rotmat_to_rot6d(polar_rotmat.permute(0, 2, 1))
            #     data_batch['polar_orient'] = polar_orient

            if 'bbox_xy' in self.head_keys:
                center = data_batch['center']
                cx = center[:, 0]
                cy = center[:, 1]
                scale = data_batch['scale'][:, 0]
                img_w = torch.max(data_batch['ori_shape'], 1)[0]
                u1 = (cx - scale / 2) / (img_w / 2) - 1
                u2 = (cx + scale / 2) / (img_w / 2) - 1
                v1 = (cy - scale / 2) / (img_w / 2) - 1
                v2 = (cy + scale / 2) / (img_w / 2) - 1
                ...
                bbox_xy = torch.cat([tx.view(-1, 1), ty.view(-1, 1)], -1)
                data_batch['bbox_xy'] = bbox_xy.float()
        ...
        if self.loss_keypoints2d_cliff is not None:
            ...
            predictions.update(
                pred_transl=pred_cam_to_transl(pred_cam, pred_focal_length *
                                               112, 224))
            predictions.update(orig_transl=pred_cam_to_full_transl(
                pred_cam, center, scale, data_batch['ori_shape'],
                orig_focal_length).float())
            ...
            if 'pred_z' in predictions:
                ...
                pred_focal_length = data_batch['smpl_transl'][:, 2].view(
                    -1) * pred_cam[:, 0]
            predictions.update(
                pred_transl=pred_cam_to_transl(pred_cam, pred_focal_length *
                                               112, 224).float())
            ...
            predictions.update(orig_transl=merge_cam_to_full_transl(
    ...
    def render_gt_iuvd_distorted(
        ...
        scale: torch.Tensor,
        orig_focal_length: torch.Tensor,
        img_res: Optional[int] = 224,
    ):
        """Compute loss for part segmentations."""
        ...
        K[:, 0, 2] = img_res / 2. - img_res * (cx - W / 2) / scale
        K[:, 1, 2] = img_res / 2. - img_res * (cy - H / 2) / scale
        cameras = build_cameras(
            dict(type='perspective',
                 image_size=(img_res, img_res),
                 in_ndc=False,
