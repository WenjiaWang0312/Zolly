...
from avatar3d.utils.eval_utils import keypoint_nme
...
class HumanImageDataset(_HumanImageDataset):
    ...
    def evaluate(self,
        ...
        for _metric in metrics:
            if _metric == 'mpjpe':
                ...
                _nv_tuples = self._report_nme(res)
            elif _metric == 'miou':
    ...
    def _report_nme(self, res_file, metric='nme', body_part=''):
        ...
        """
        pred_keypoints2d, gt_keypoints2d, gt_keypoints2d_mask = \
            self._parse_result2d(res_file)

        err_name = metric.upper()
        if body_part != '':
            err_name = body_part.upper() + ' ' + err_name

        error = keypoint_nme(pred_keypoints2d, gt_keypoints2d,
                             gt_keypoints2d_mask)
