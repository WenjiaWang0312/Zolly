...
from avatar3d.models.body_models.mappings import get_flip_pairs, convert_kps
...
from mmhuman3d.data.datasets.pipelines.transforms import (
    ...
    affine_transform)
from avatar3d.utils.bbox_utils import kp2d_to_bbox
...
class MeshAffine:
    ...
    def __call__(self, results):
        ...
        if self.erase_background:
            ...
            new_ori_img = np.random.randint(0, 255, ori_img.shape).astype(np.uint8)
            left, top, right, bottom = xyxy.reshape(4).tolist()
            left, top, right, bottom = int(left), int(top), int(right), int(bottom)
            new_ori_img[top:bottom, left:right] = ori_img[top:bottom, left:right]
            ori_img = new_ori_img
        for key in results.get('img_fields', self.img_fields):
            if key == 'ori_img':
        ...
        if 'keypoints2d' in results:
            ...
            for part_name in self.convention_dst:
                keypoints2d_part, _ = convert_kps(origin_keypoints2d[None],
                                                self.convention_src,
        ...
        if 'keypoints3d' in results:
            ...
            for part_name in self.convention_dst:
                keypoints3d_part, _ = convert_kps(keypoints3d[None],
                                                self.convention_src,
                                                self.convention_dst[part_name])
        ...
        if 'keypoints2d' in results:
            ...
            for part_name in ['lhand', 'rhand', 'face']:
                if f'keypoints2d_{part_name}' in results:
                    keypoints2d_part = results[f'keypoints2d_{part_name}'].copy()
                    keypoints2d_part[:, :2] = (np.dot(keypoints2d_part[:, :2], M[:2, :2].T) +
                                    M[:2, 2] + 1).astype(np.int)
                    results[f'keypoints2d_{part_name}'] = keypoints2d_part
        ...
        if 'keypoints3d' in results:
            ...
            for part_name in ['lhand', 'rhand', 'face']:
                if f'keypoints3d_{part_name}' in results:
                    keypoints3d_part = results[f'keypoints3d_{part_name}'].copy()
                    keypoints3d_part[:, :3] = _rotate_joints_3d(keypoints3d_part[:, :3], r)
...
class GenHeatmap(object):
    ...
    def __call__(self, results):
        ...
        for part_name, img_name in self.img_name_src.items():
            ...
            target_weight = np.ones((num_joints, 1), dtype=np.float32) * mask.reshape(-1, 1)
